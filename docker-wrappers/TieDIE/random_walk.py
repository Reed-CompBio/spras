from random_walk_util import pagerank
...
def parse_arguments():
    ...
    parser.add_argument("--network", type=Path, required=True, help="Placeholder")
...
def random_walk(r = 'pagerank', G, S, T, f = 'min', sigma):
    if r == 'pagerank':
        s1 = pagerank(G, S, alpha=0.85, personalization=None, max_iter=100, tol=1e-06, nstart=None, weight='weight', dangling=None)
        t1 = pagerank(G.reverse(), T, alpha=0.85, personalization=None, max_iter=100, tol=1e-06, nstart=None, weight='weight', dangling=None)
        for i in G.nodes():
            z_score = f(s1[i], t1[i])
            if sigma > z_score:
                G.remove_node(i)
