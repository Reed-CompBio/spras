...
from spras.diamond import DIAMOnD, DIAMOnDParams
...
class TestDIAMOnD:
    ...
    def test_diamond_medium(self):
        ...
        DIAMOnD.run({"network": TEST_DIR / 'input' / 'diamond-medium-network.txt',
                     "seeds": TEST_DIR / 'input' / 'diamond-medium-seeds.txt'},
                     output_file=OUT_FILE,
                     args=DIAMOnDParams(n=200))
        assert OUT_FILE.exists()

    def test_diamond_required(self):
        OUT_FILE.unlink(missing_ok=True)
        DIAMOnD.run({"network": TEST_DIR / 'input' / 'diamond-network.txt',
                     "seeds": TEST_DIR / 'input' / 'diamond-seeds.txt'},
                     output_file=OUT_FILE,
                     args=DIAMOnDParams(n=2))
        assert OUT_FILE.exists()
    ...
    def test_diamond_optional(self):
        ...
        DIAMOnD.run({"network": TEST_DIR / 'input' / 'diamond-network.txt',
                     "seeds": TEST_DIR / 'input' / 'diamond-seeds.txt'},
                     output_file=OUT_FILE,
                     args=DIAMOnDParams(alpha=0, n=2))
        assert OUT_FILE.exists()
    ...
    def test_domino_missing_network(self):
        ...
        with pytest.raises(ValueError):
            # No network
            DIAMOnD.run({"seeds": TEST_DIR / 'input' / 'diamond-seeds.txt'},
                        output_file=OUT_FILE,
                        args=DIAMOnDParams(n=2))
