...
class TestEvaluate:
    ...
    def test_node_precision_recall_pca_chosen_pathway_not_provided(self):
        output_file = OUT_DIR / 'pr-per-pathway-pca-chosen-not-provided.txt'
        output_file.unlink(missing_ok=True)
        output_png = OUT_DIR / 'pr-per-pathway-pca-chosen-not-provided.png'
        output_png.unlink(missing_ok=True)
        file_paths = []
    ...
    def test_node_precision_recall_pca_chosen_pathway(self):
        output_file = OUT_DIR / 'pr-per-pathway-pca-chosen.txt'
        output_file.unlink(missing_ok=True)
        output_png = OUT_DIR / 'pr-per-pathway-pca-chosen.png'
        output_png.unlink(missing_ok=True)
        output_coordinates = OUT_DIR / 'pca-coordinates.tsv'
        ...
        ml.pca(dataframe, str(OUT_DIR / 'pca.png'), str(OUT_DIR / 'pca-variance.txt'), str(output_coordinates), kde=True, remove_empty_pathways=True)
    ...
    def test_node_ensemble(self):
        ...
        node_ensemble_dict = Evaluation.edge_frequency_node_ensemble(GS_NODE_TABLE, ensemble_network, str(input_network))
    ...
    def test_multiple_node_ensemble(self):
        ...
        input_network = OUT_DIR / 'data.pickle'
        node_ensemble_dict = Evaluation.edge_frequency_node_ensemble(GS_NODE_TABLE, ensemble_networks, str(input_network))
        node_ensemble_dict['ensemble'].to_csv(out_path_file, sep='\t', index=False)
        assert filecmp.cmp(out_path_file, EXPECT_DIR / 'expected-node-ensemble.csv', shallow=False)
    ...
    def test_precision_recall_curve_ensemble_nodes(self):
        out_path_png = OUT_DIR / 'pr-curve-ensemble-nodes.png'
        out_path_png.unlink(missing_ok=True)
        out_path_file = OUT_DIR / 'pr-curve-ensemble-nodes.txt'
        out_path_file.unlink(missing_ok=True)
    ...
    def test_precision_recall_curve_ensemble_nodes_empty(self):
        out_path_png = OUT_DIR / 'pr-curve-ensemble-nodes-empty.png'
        out_path_png.unlink(missing_ok=True)
        out_path_file = OUT_DIR / 'pr-curve-ensemble-nodes-empty.txt'
        out_path_file.unlink(missing_ok=True)
    ...
    def test_precision_recall_curve_multiple_ensemble_nodes(self):
        out_path_png = OUT_DIR / 'pr-curve-multiple-ensemble-nodes.png'
        out_path_png.unlink(missing_ok=True)
        out_path_file = OUT_DIR / 'pr-curve-multiple-ensemble-nodes.txt'
        out_path_file.unlink(missing_ok=True)
        ensemble_file = pd.read_csv(INPUT_DIR / 'node-ensemble.csv', sep='\t', header=0)
        empty_ensemble_file = pd.read_csv(INPUT_DIR / 'node-ensemble-empty.csv', sep='\t', header=0)
