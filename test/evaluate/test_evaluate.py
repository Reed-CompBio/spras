...
class TestEvaluate:
    ...
    def test_precision_recall_pca_chosen_pathway(self):
        ...
        output_png = OUT_DIR / "test-pr-per-pathway-pca-chosen.png"
        ...
        ml.pca(dataframe, str(OUT_DIR / 'pca.png'), str(OUT_DIR / 'pca-variance.txt'), output_coordinates, kde=True, remove_empty_pathways=True)

        pathway = Evaluation.pca_chosen_pathway([output_coordinates], str(SUMMARY_FILE), str(INPUT_DIR))
    ...
    def test_precision_recall_pca_chosen_pathway_not_provided(self):
        ...
        output_png = OUT_DIR / "test-pr-per-pathway-pca-chosen-not-provided.png"
        output_png.unlink(missing_ok=True)
