...
def concat_dfs(list_of_dataframes):
    ...
    result = result.fillna(0) #result.fillna('-') 
...
def parse_files(edge_files):
    ...
    for file in edge_files:
        ...
        parts = file.split('/')
        print (parts[-2])
        list_of_tuples.append((parts[-2], result))
...
def main(args):
    ...
    con_dfs = con_dfs.astype('int')
    
    # con_dfs= con_dfs.reset_index(drop=True)
    print(con_dfs)
    con_dfs.to_csv('concatdf.csv',index=False)
...
if __name__ == "__main__":
    ...
    # python similarity_matrix.py --edge_files /Users/nehatalluri/Desktop/jobs/research/spras/output/data0-mincostflow-params-SZPZVU6/pathway.txt
